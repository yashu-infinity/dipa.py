...
FAMAPI_API_KEY = os.getenv("FAMAPI_API_KEY", "FAM_LIVE_sk_MRmsER47dffeEMClg7lwXWw4v1OJwjfu").strip()
...
def setup_db():
    with db() as con:
        ...
        cur.execute("INSERT OR IGNORE INTO admins (chat_id) VALUES (?)", (ADMIN_CHAT_ID,))
...
async def create_famapi_order(amount_rupees, chat_id, user_name="User", bot=None):
    try:
        ...
        logger.info(f"FamAPI create order: {response.status_code} - {response.text[:200]}")
    ...
    try:
        ...
        try:
            if not QR_LIBS_OK or qrcode is None:
                raise RuntimeError("QR libraries are not installed. Install requirements.txt (qrcode[pil]).")
...
# ============================================================
# PAYMENT PHOTO (MANUAL APPROVAL BACKUP)
# ============================================================
